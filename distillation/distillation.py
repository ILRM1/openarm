...
class Dagger:
    def __init__(self, env, config, summaries_dir, nn_dir, distributed):
        ...
        self.num_actions = self.ov_env.cfg.num_actions
        ...
        self.student_model_config = {
            ...
            "input_shape": (self.ov_env.cfg.num_observations,),
        ...
        self.teacher_model = Agent(self.env.unwrapped).to(self.device)
    ...
    def init_tensors(self):
        ...
        self.lstm_state = (
            torch.zeros(self.teacher_model.lstm.num_layers, self.num_envs, self.teacher_model.lstm.hidden_size, device=self.device),
            torch.zeros(self.teacher_model.lstm.num_layers, self.num_envs, self.teacher_model.lstm.hidden_size, device=self.device),
        )
    ...
    def get_actions(self, obs, policy_type):
        ...
        if policy_type == "student":
            ...
            action, mus, sigmas, self.lstm_state = self.teacher_model.get_action_mu_sigma(obs["expert_policy"], self.lstm_state, self.dones)
    ...
    def set_weights(self, ckpt, policy_type):
        ...
        if policy_type == "student":
            ...
            model.load_state_dict(weights["agent"])
