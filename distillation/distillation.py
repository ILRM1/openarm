...
from cleanrl.cleanrl.ppo_openarm_lstm import Agent
...
class Dagger:
    def __init__(self, env, config, summaries_dir, nn_dir):
        ...
        self.agent = Agent(self.env.unwrapped).to(self.device)
        self.agent.load_state_dict(torch.load(self.config["teacher"]["ckpt"], map_location=self.device))
        self.agent.eval()
    ...
    def init_tensors(self):
        ...
        self.lstm_state = (
            torch.zeros(self.agent.lstm.num_layers, self.num_envs, self.agent.lstm.hidden_size, device=self.device),
            torch.zeros(self.agent.lstm.num_layers, self.num_envs, self.agent.lstm.hidden_size, device=self.device),
        )
    ...
    def get_actions(self, obs, policy_type):
        ...
        if policy_type == "student":
            ...
            action, mus, sigmas, self.lstm_state = self.agent.get_action_mu_sigma(obs["expert_policy"], self.lstm_state, self.dones)
