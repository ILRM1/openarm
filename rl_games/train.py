...
# parse the arguments
parser.add_argument("--depth", action="store_true", default=False, help="Enable depth-based teacher training.")

args_cli, hydra_args = parser.parse_known_args()
# always enable cameras to record video
if args_cli.video:
    args_cli.enable_cameras = True
# enable cameras for depth-based training
if args_cli.depth:
    args_cli.enable_cameras = True
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: dict):
    ...
    # Enable depth teacher mode before env creation
    if args_cli.depth:
        env_cfg.use_depth_teacher = True
        print("[INFO]: Depth-based teacher training enabled. Camera will be activated.")
    ...
    # Register depth network and agent if depth training is enabled
    if args_cli.depth:
        from depth_network import DepthA2CBuilder
        from depth_ppo_agent import DepthA2CAgent
        model_builder.register_network('depth_a2c_network', DepthA2CBuilder)
        runner.algo_factory.register_builder(
            'a2c_continuous',
            lambda **kwargs: DepthA2CAgent(**kwargs),
        )
    ...
    # train the agent
    if args_cli.checkpoint is not None:
        runner.run({"train": False, "play": True, "sigma": train_sigma, "checkpoint": resume_path})
    else:
