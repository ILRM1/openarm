...
class IKSolver6D:
    ...
    def solve(self, target: pin.SE3, q_init: np.ndarray):
        ...
        for _ in range(self.max_iter):
            ...
            # Isaac Lab compute_pose_error(rot_error_type="axis_angle") 방식:
            # LOCAL_WORLD_ALIGNED Jacobian과 동일한 월드 프레임으로 오차 계산
            err_pos = target.translation - T_ee.translation
            err_rot = pin.log3(target.rotation @ T_ee.rotation.T)
            ...
            # DLS (Isaac Lab differential_ik.py dls 방식과 동일)
            lambda_matrix = (self.damping ** 2) * np.eye(6)
            dq = J.T @ np.linalg.solve(J @ J.T + lambda_matrix, err)
...
class OpenArmDualIKNode(Node):
    ...
    def _setup_pubsub(self):
        ...
        self._pubs["dual/joint_states"] = self.create_publisher(
            JointState, "/arm", qos_r)
    ...
    def _ik_tick(self):
        ...
        for side in Side:
            ...
            with state.lock:
                if state.target is None:
                    # target 없는 팔도 현재 q로 results에 포함 → dual 항상 발행 가능
                    results[side] = (state.q.copy(), True, 0.0)
                    continue
                target = state.target
                q_arm  = state.q.copy()
        ...
        if len(results) == 2:
            ...
            self._pub_dual_joint_states(results)
    ...
    def _pub_arm(self, side: Side, q: np.ndarray, ok: bool, err: float):
        ...
        st = String()
        st.data = f"{'OK' if ok else 'FAIL'} | err={err:.6f}"
        self._pubs[f"{ns}/status"].publish(st)

    def _pub_dual_joint_states(self, results: dict):
        js = JointState()
        ...
        js.velocity = []
        js.effort   = []
        self._pubs["dual/joint_states"].publish(js)
