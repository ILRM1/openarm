...
class OpenarmEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: OpenarmEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        self.left_arm_joint_id = self.actuated_dof_indices[:6].copy()
        self.right_arm_joint_id = self.actuated_dof_indices[6:12].copy()
        self.left_gripper_joint_id = self.actuated_dof_indices[12:28].copy()
        self.right_gripper_joint_id = self.actuated_dof_indices[28:44].copy()

        self.grasp_joint_pos = torch.ones(len(self.left_gripper_joint_id), device=self.device) * torch.pi/2.
        self.grasp_joint_pos[12:] = torch.tensor([1.43, 0.94, 0.22, 0.55], device=self.device)
        ...
        self.object_goal = torch.tensor([0.25, 0.15, 0.1], device=self.device).repeat((self.num_envs, 1))
        ...
        self.joint_pos_des = self.robot_start_joint_pos[:, :6].clone()
...
#                torch.ones(self.num_envs, 1, device=self.device)
    ...
    def _pre_physics_step(self, actions: torch.Tensor) -> None:
        ...
        self.raw_gipper_action = actions[:,6].clone()
        ...
        delta_axis = F.normalize(delta_rotvec, dim=-1)
        ...
        # self.left_gripper_action = torch.tensor([0.] * self.num_envs, device=self.device)
        self.control_gripper_joint_pos = torch.where((self.left_gripper_action>0.5).unsqueeze(-1), 0., self.grasp_joint_pos)
    ...
    def _get_observations(self) -> dict:
        ...
        if self.cfg.distillation:
            ...
            observations = {
                "policy": policy_obs,
                "rgb": torch.cat([head_depth, wrist_L_depth], dim=1),  # (N, 2, H, W)
            }
        else:
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        if self.common_step_counter % 10 == 0:
            ...
            print("lift weight: %0.1f" % self.dextrah_adr.get_custom_param_value("reward_weights", "lift_weight"))
            print(self.grasp_pos[0])
            print(self.left_tcp_pose[0, :3])
            print("------------------------")
...
#            False)
    ...
    def compute_intermediate_reward_values(self):
        ...
        self.hand_to_object_pos_error = torch.norm(self.left_tcp_pose[:,:3] - self.grasp_pos, dim=-1)
...
def compute_rewards(
    ...
    close_gripper_reward = 0.7*torch.where(hand_to_object_pos_error<=0.02, torch.clamp(torch.exp(-1. * raw_gipper_action), max=3.), 0.)
    close_gripper_penalty = 0.5*torch.exp(-10. * hand_to_object_pos_error)*torch.where(((hand_to_object_pos_error>0.02)) & (gripper_action<=0.5), -1., 0.)
    
    # Reward for lifting object off table and towards object goal
    lift_reward = 5. * torch.exp(-10. * object_vertical_error)
    lift_reward = torch.where((object_pos[:,2]>0.02)&(gripper_action<=0.5)&(hand_to_object_pos_error<=0.02), lift_reward, 0.)
