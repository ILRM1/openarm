...
from .bili_elevator_env_cfg import BiliElevatorEnvCfg


# ADR imports
from ..openarm.dextrah_adr import DextrahADR

class BiliElevatorEnv(DirectRLEnv):
    cfg: BiliElevatorEnvCfg

    @staticmethod
    def _find_shader_prim(prim):
        if prim.GetTypeName() == "Shader":
            return prim
        for child in prim.GetChildren():
            result = BiliElevatorEnv._find_shader_prim(child)
            if result is not None:
                return result
        return None

    def __init__(self, cfg: BiliElevatorEnvCfg, render_mode: str | None = None, **kwargs):
        ...
        for joint_name in (self.cfg.leg_joint_name+self.cfg.head_joint_name+self.cfg.body_joint_name):
            self.body_dof_indices.append(self.robot.joint_names.index(joint_name))

        self.robot_start_body_joint_pos = self.robot.data.default_joint_pos[:, self.body_dof_indices]
        self.robot_body_dof_lower_limits = joint_pos_limits[..., 0][:, self.body_dof_indices]
        self.robot_body_dof_upper_limits = joint_pos_limits[..., 1][:, self.body_dof_indices]
        self.robot_start_body_joint_pos = torch.clamp(self.robot_start_body_joint_pos, min=self.robot_body_dof_lower_limits[0], max=self.robot_body_dof_upper_limits[0])
        self.robot_start_body_joint_vel = torch.zeros(self.robot_start_body_joint_pos.shape, device=self.device)
        ...
        self.robot_start_joint_pos = self.robot.data.default_joint_pos[:, self.actuated_dof_indices]
        ...
        # self.joint_pos_des_delta_limit = 0.03
        ...
        self.hand_to_object_pos_error_2d = torch.ones(self.num_envs, device=self.device)
        self.force_goal_error = torch.ones(self.num_envs, device=self.device)
        ...
        self.right_tcp_pos_bias_width = torch.zeros(self.num_envs, 1, device=self.device)
        self.right_tcp_ori_bias_width = torch.zeros(self.num_envs, 1, device=self.device)
        self.right_tcp_pos_bias = torch.zeros(self.num_envs, 1, device=self.device)
        self.right_tcp_ori_bias = torch.zeros(self.num_envs, 1, device=self.device)
        self.right_tcp_pos_noise_width = torch.zeros(self.num_envs, 1, device=self.device)
        self.right_tcp_ori_noise_width = torch.zeros(self.num_envs, 1, device=self.device)
        ...
        # self.cam_frame1_prims = []
        # self.cam_frame1_orig_pos = []
        # for i in range(self.num_envs):
        #     prim = _stage.GetPrimAtPath(f"/World/envs/env_{i}/Robot/openarm_left_link7/cam_frame1")
        #     self.cam_frame1_prims.append(prim)
        #     orig = Gf.Vec3d(0, 0, 0)
        #     for op in UsdGeom.Xformable(prim).GetOrderedXformOps():
        #         if op.GetOpType() == UsdGeom.XformOp.TypeTranslate:
        #             orig = op.Get()
        #             break
        #     self.cam_frame1_orig_pos.append(orig)
        ...
        # self.right_gripper_action = torch.ones(self.num_envs, device=self.device)
        ...
        # Position-only IK: orientation moves naturally (not constrained)
        self.diff_ik_cfg_pos = DifferentialIKControllerCfg(command_type="position", use_relative_mode=False, ik_method="dls")
        self.diff_ik_controller_pos = DifferentialIKController(self.diff_ik_cfg_pos, num_envs=self.num_envs, device=self.device)

        self.pre_actions = torch.zeros(self.num_envs, 6, device=self.device)
        self.pre_left_tcp_pose = torch.zeros(self.num_envs, 6, device=self.device)

        self.reach_success = torch.zeros(self.num_envs, device=self.device, dtype=torch.bool)
        self.press_success = torch.zeros(self.num_envs, device=self.device, dtype=torch.bool)
...
#                torch.ones(self.num_envs, 1, device=self.device)
        ...
        for i in range(self.num_envs):
            ...
            #self.multi_object_idx = torch.tensor([6], device=self.device)
            ...
            object_cfg = RigidObjectCfg(
                ...
                spawn=sim_utils.UsdFileCfg(
                    ...
                    mass_props=sim_utils.MassPropertiesCfg(density=8.0),
                ),
            ...
            self.object_mat_prims.append(self._find_shader_prim(prim))
    ...
    def _pre_physics_step(self, actions: torch.Tensor) -> None:
        ...
        # left_target_pos = self.left_tcp_pose[:,:3] + self.actions[:,:3] * self.action_scale[0]
        # curr_quat = quat_from_euler_xyz(self.left_tcp_pose[:,3], self.left_tcp_pose[:,4], self.left_tcp_pose[:,5])
        # delta_rotvec = self.actions[:,3:6]  * self.action_scale[1]
        # delta_angle = torch.norm(delta_rotvec, dim=-1).clamp(min=1e-6)
        # delta_axis  = delta_rotvec / delta_angle.unsqueeze(-1)
        ...
        # self.diff_ik_controller.set_command(left_target_pose)
        # self.joint_pos_des_new = self.compute_ik(self.left_tcp_id, self.left_arm_joint_id)

        right_target_pos = self.right_tcp_pose[:,:3] + self.actions[:,:3] * self.action_scale[0]
        curr_quat = quat_from_euler_xyz(self.right_tcp_pose[:,3], self.right_tcp_pose[:,4], self.right_tcp_pose[:,5])
        delta_rotvec = self.actions[:,3:6]  * self.action_scale[1]
        delta_angle = torch.norm(delta_rotvec, dim=-1).clamp(min=1e-6)
        delta_axis  = delta_rotvec / delta_angle.unsqueeze(-1)
        delta_quat = quat_from_angle_axis(delta_angle, delta_axis)
        right_target_quat = quat_mul(delta_quat, curr_quat)

        root_offset = self.robot.data.root_pos_w - self.scene.env_origins
        ...
        self.joint_pos_des = torch.cat((arm_joint_pos_des, self.right_finger_joint_pos, self.robot_start_body_joint_pos), dim=-1)

    def _apply_action(self) -> None:
        self.robot.set_joint_position_target(self.joint_pos_des, joint_ids=self.right_arm_joint_id+self.right_gripper_joint_id+self.body_dof_indices)
        self.robot.set_joint_velocity_target(self.robot_start_body_joint_vel, joint_ids=self.body_dof_indices)
    ...
    def _get_observations(self) -> dict:
        ...
        if self.cfg.distillation:
            ...
            head_depth = F.interpolate(
                head_depth, size=(self.cfg.distil_depth_height, self.cfg.distil_depth_width),
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        #smooth = -0.0001 * torch.sum(torch.square(self.actions[:, :6] - self.pre_actions), dim=1)
    ...
    def _get_dones(self) -> torch.Tensor:
        ...
        #self.pre_left_tcp_pose = self.left_tcp_pose_noisy.clone()
        self.pre_right_tcp_pose = self.right_tcp_pose_noisy.clone()
        ...
        # self.out_of_joint_limit = self.robot_dof_pos[:,3] <= self.robot_dof_lower_limits[0, 3] + 0.05
    ...
    def _reset_idx(self, env_ids: Sequence[int] | None):
        ...
        z_width_spawn = self.dextrah_adr.get_custom_param_value("object_spawn", "z_width_spawn")
...
#        )
        ...
        z_angle = sample_uniform(-rotation, rotation, (num_ids,), device=self.device)
        ...
        # Now clamp
        dof_pos = torch.clamp(dof_pos,min=self.robot_dof_lower_limits[0],max=self.robot_dof_upper_limits[0])
        ...
        self.robot.write_joint_state_to_sim(dof_pos, dof_vel, env_ids=env_ids, joint_ids=self.actuated_dof_indices+self.body_dof_indices)
        self.robot.set_joint_position_target(dof_pos, env_ids=env_ids, joint_ids=self.actuated_dof_indices+self.body_dof_indices)
        self.robot.set_joint_velocity_target(dof_vel, env_ids=env_ids, joint_ids=self.actuated_dof_indices+self.body_dof_indices)
        ...
        self.robot_material_props = self.robot.root_physx_view.get_material_properties().to(device=self.device).view(self.num_envs, -1)
...
#                    input(self.robot.root_physx_view.get_dof_friction_coefficients())
        ...
        self.right_tcp_pos_bias_width[env_ids, 0] = 0.01 * torch.rand(num_ids, device=self.device)
        self.right_tcp_ori_bias_width[env_ids, 0] = 0.1 * torch.rand(num_ids, device=self.device)
        self.right_tcp_pos_bias[env_ids, 0] = self.right_tcp_pos_bias_width[env_ids, 0] * (torch.rand(num_ids, device=self.device) - 0.5)
        self.right_tcp_ori_bias[env_ids, 0] = self.right_tcp_ori_bias_width[env_ids, 0] * (torch.rand(num_ids, device=self.device) - 0.5)
        self.right_tcp_pos_noise_width[env_ids, 0] = 0.01 * torch.rand(num_ids, device=self.device)
        self.right_tcp_ori_noise_width[env_ids, 0] = 0.1 * torch.rand(num_ids, device=self.device)
...
#            False)
        ...
        # # Randomize cam_frame1 local X and Z position by ±1cm
        # for env_id in env_ids.cpu().numpy():
        #     xformable = UsdGeom.Xformable(self.cam_frame1_prims[env_id])
        #     orig = self.cam_frame1_orig_pos[env_id]
        #     noise_x = np.random.uniform(-0.015, 0.015)
        #     noise_z = np.random.uniform(-0.015, 0.015)
        #     for op in xformable.GetOrderedXformOps():
        #         if op.GetOpType() == UsdGeom.XformOp.TypeTranslate:
        #             op.Set(Gf.Vec3d(orig[0] + noise_x, orig[1], orig[2] + noise_z))
        #             break
        ...
        # self.left_gripper_action = torch.ones(self.num_envs, device=self.device, dtype=self.left_gripper_action.dtype)
        # self.right_gripper_action = torch.ones(self.num_envs, device=self.device, dtype=self.right_gripper_action.dtype)
        ...
        #self.joint_pos_des[env_ids] = self.robot_dof_pos[env_ids].to(dtype=self.joint_pos_des.dtype)
        #self.pre_left_tcp_pose[env_ids] = self.left_tcp_pose_noisy[env_ids].clone()

        self.reach_success[env_ids] = torch.zeros(num_ids, device=self.device, dtype=torch.bool)
        self.press_success[env_ids] = torch.zeros(num_ids, device=self.device, dtype=torch.bool)
    ...
    def _compute_intermediate_values(self):
        ...
        self.robot_dof_pos = self.robot.data.joint_pos[:, self.right_arm_joint_id]
        ...
        self.robot_dof_vel = self.robot.data.joint_vel[:, self.right_arm_joint_id]
        ...
        self.right_tcp_pose = self.robot.data.body_pose_w[:, self.right_tcp_id].clone()
        self.right_tcp_pose[:, :3] = self.right_tcp_pose[:, :3] - self.scene.env_origins
        right_tcp_euler = torch.stack(euler_xyz_from_quat(self.right_tcp_pose[:, 3:], wrap_to_2pi = False), dim=-1)
        self.right_tcp_pose = torch.cat((self.right_tcp_pose[:, :3], right_tcp_euler), dim=-1)

        right_tcp_pos_noisy = self.right_tcp_pose[:, :3] + self.right_tcp_pos_noise_width  * 2. * (torch.rand_like(self.right_tcp_pose[:, :3]) - 0.5) + self.right_tcp_pos_bias
        right_tcp_euler_noisy = right_tcp_euler + self.right_tcp_ori_noise_width * 2. * (torch.rand_like(right_tcp_euler) - 0.5) + self.right_tcp_ori_bias
        self.right_tcp_pose_noisy = torch.cat((right_tcp_pos_noisy, right_tcp_euler_noisy), dim=-1)
        ...
        self.right_index_tip_contact_force = self.right_index_tip_contact_sensor.data.net_forces_w[:,0,0]
    ...
    def compute_intermediate_reward_values(self):
        ...
        self.hand_to_object_pos_error_2d = torch.norm(self.right_tcp_pose[:,:2] - self.object_pos[:,:2], dim=-1)
    ...
    def compute_ik_pos_only(self, ee_link_id, arm_joint_id, target_pos_b: torch.Tensor):
        """Position-only IK: only XYZ target (root frame), orientation moves naturally.

        Args:
            target_pos_b: Desired EE position in robot root frame, shape (N, 3).
        """
        jacobian = self.robot.root_physx_view.get_jacobians()[:, ee_link_id-1, :, arm_joint_id]
        ee_pose_w = self.robot.data.body_pose_w[:, ee_link_id]
        root_pose_w = self.robot.data.root_pose_w
        ...
        # ee_quat_b required by set_command even in position mode (stores current orientation for display)
        self.diff_ik_controller_pos.set_command(target_pos_b, ee_quat=ee_quat_b)
        joint_pos_des = self.diff_ik_controller_pos.compute(ee_pos_b, ee_quat_b, jacobian, joint_pos)
        return joint_pos_des
...
def compute_rewards(
    ...
    hand_to_object_reward = torch.where(reach_success, 0., hand_to_object_reward)
    
    press_reward = 3. * torch.exp(-15. * force_goal_error)
    press_reward = torch.where((hand_to_object_pos_error_2d<=0.01) & reach_success, press_reward, 0.)
    press_reward = torch.where(press_success, 0., press_reward)
